 ...
 */
 ...
 const USER_AGENT = 'github.com/tbo47/ez-opendata';
 const OPTIONS_WITH_USER_AGENT = { headers: { 'User-Agent': USER_AGENT } };
 ...
 */
 const openstreetmapGetPOIs = async (bbox = '37.8,-122.3,37.8,-122.2', categories = OSM_CATEGORIES.sustenance, timeout = 25_000) => {
 ...
 * ```
 */
 const openstreetmapGetPOIsBbox = async ({ _northEast, _southWest, }, categories = OSM_CATEGORIES.food) => {
    const bbox = [];
 ...
 */
 const getFoodShops = async ({ _northEast, _southWest, }) => {
 ...
 */
 const wikipediaQuery = async (lat = 37, lon = -122, language = 'en', radius = 10_000, limit = 100) => {
 ...
 const wikidataQuery = async (northEast, southWest, limit = 3000) => {
 ...
 const wikimediaQueryBound = async (bounds, limit = 5000) => {
    return await wikimediaQuery(bounds.getNorthEast(), bounds.getSouthWest(), limit);
};
 const wikimediaQuery = async (northEast, southWest, limit = 100) => {
 ...
 */
 const wikimediaGetThumbs = async (pageids, orientation, value = 600) => {
 ...
 */
 const wikimediaInfoMultiplePages = wikimediaGetThumbs;
/**
 ...
 * @returns
 */
 const wikimediaGetThumb = async (pageid, height, width) => {
 ...
 const wikimediaGetAuthor = async (title, pageid) => {
 ...
 */
 const wikimediaPicOfTheDay = async (lang = '') => {
    if (!lang) {
        lang = getLang();
    }
